...
class MaximumFileSize extends AbstractValidator
{
    ...
    public function __construct(\Foundation\Form\Element $element, $sizeInBytes)
    {
        if (!$integer = \intval($sizeInBytes)) {
            throw new \Foundation\Exception(
                "The ruleset for MaximumFileSize must be an integer. Value: '{$sizeInBytes}'"
            );
        }
    ...
    public function validate(\Foundation\Form\Input $input)
    {
        ...
        if ($fileArr['size'] > $this->ruleSet) {
            $this->addError(
                'File is too large.  Your file is: ' .
                \Foundation\Utility::convertBytesToString($fileArr['size'] - $this->ruleSet) .
                ' bigger than the maximum size of ' .
                \Foundation\Utility::convertBytesToString($this->ruleSet, 0)
            );
    ...
    public function preRender()
    {
        if (is_null($this->e->getFormat())) {
            $this->e->setFormat(
                'Maximum file size: ' . \Foundation\Utility::convertBytesToString($this->ruleSet, 2) . '.'
            );
        }
    }
}
