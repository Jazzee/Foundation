...
class RankingList extends AbstractElement
{
    ...
    public function preRender()
    {
        if (!$this->totalItems or !$this->requiredItems) {
            throw new \Foundation\Exception(
                'RankingListElement requires total items and required items to be set before it is rendered.'
            );
        }
        parent::preRender();
    }
    ...
    {
        if (array_key_exists($listItem->getValue(), $this->items)) {
            throw new \Foundation\Exception($listItem->getValue() . ' is already an item in this list');
        }
