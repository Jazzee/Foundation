...
class FileInput extends Input
{
            ...
            if ($fileArr['error'] != \UPLOAD_ERR_OK) {
                switch ($fileArr['error']) {
                    ...
                    case \UPLOAD_ERR_FORM_SIZE:
                        $text = 'Your file is greater than the maximum allowed size of ' .
                            \Foundation\Utility::convertBytesToString($this->getMaxSize());
                        break;
                    case \UPLOAD_ERR_PARTIAL:
                        $text = 'Your file upload was stopped before it completed.  ' .
                            'This is probably a temporary problem with your connection to our server.  ' .
                            'Please try again.';
                        break;
                    ...
                    case UPLOAD_ERR_NO_TMP_DIR:
                        throw new \Foundation\Exception(
                            'Unable to upload file: no temporary directory was found.',
                            \E_USER_ERROR,
                    ...
                    case UPLOAD_ERR_CANT_WRITE:
                        throw new \Foundation\Exception(
                            'Unable to upload file: could not write to disk.',
                            E_USER_ERROR,
                            'The server encountered an error uploading your file.  Please try again.'
                        );
                        break;
                    case \UPLOAD_ERR_EXTENSION:
                        throw new \Foundation\Exception(
                            'Unable to upload file: A PHP extension stopped the file upload. ' .
    ...
    public function setType($type)
    {
        if ($type != 'file') {
            throw new \Foundation\Exception(
                "A type of {$type} is not allowed.  Only 'file' is allowed for this element"
            );
        }
    }
    ...
    public function setMaxSize($maxSize)
    {
        ...
        if ($maxSize > \Foundation\Utility::convertIniShorthandValue(\ini_get('upload_max_filesize'))) {
            throw new \Foundation\Exception(
                'Attempting to set FileInput::maxSize to a value greater than PHP INI upload_max_filesize'
            );
        }
        if ($maxSize > \Foundation\Utility::convertIniShorthandValue(\ini_get('post_max_size'))) {
            throw new \Foundation\Exception(
                'Attempting to set FileInput::maxSize to a value greater than PHP INI post_max_size'
            );
        }
