...
class Configuration{
  ...
  public function setCacheType($type){
    if(!in_array($type, array('array','apc'))) throw new Exception("{$type} is not a valid cache type");
    $this->cacheType = $type;
  }
  /**
   * Get the cache type
  ...
  public function setMailDefaultFromAddress($address){
    if($address AND !\filter_var($address, FILTER_VALIDATE_EMAIL)) throw  new Exception("{$address} is not a valid email address");
...
/**
  ...
  public function setMailServerType($type){
    if(!in_array($type, array('php','sendmail','smtp','smtp+ssl'))) throw new Exception("{$type} is not a valid Mail Server type");
